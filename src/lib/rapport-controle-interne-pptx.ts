...
type Strings = {
  title: string; subtitle: string; ligne: Record<LigneDefense, string>; ligneFusionnee: string
...
export async function renderRapportControleInternePptx(
  ...
  for (const g of rapport.groupes) {
    ...
    sl.addText(rapport.ligneUnique && g.ligne === '1' ? S.ligneFusionnee : (S.ligne[g.ligne] ?? g.ligne), { x: 0.5, y: 0.4, w: 12.3, h: 0.6, fontSize: 20, bold: true, color: COLORS.primary })
