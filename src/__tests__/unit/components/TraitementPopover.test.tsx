...
describe('TraitementPopover — rattacher/mettre à jour', () => {
  ...
  it('crée un NOUVEAU plan d\'action = un vrai PlanAction (POST plans-actions + lien CONFORMITE + priorité)', async () => {
    ...
    expect(body.priorite).toBe('MAJEUR')
    expect(body.liens).toEqual([{ type: 'CONFORMITE', targetId: 'ISO27001', ref: 'A.5.1', label: 'Politique' }])
  })

  it('acceptation : sélectionner un traitement existant → PATCH conformité + addRef (libellé verrouillé)', async () => {
    const fetchMock = mockFetch()
    vi.stubGlobal('fetch', fetchMock)
    const onApplied = vi.fn()
    render(<TraitementPopover
      orgId="o1" referentiel="ISO27001" entite="" controlRef="A.5.1" controlNom="Politique"
      type="ACCEPTATION_RISQUE" existing={existing} onApplied={onApplied} onClose={() => {}} />)
    fireEvent.focus(screen.getByLabelText('Intitulé'))
    fireEvent.mouseDown(screen.getByText('Acceptation legacy'))
    ...
    const patch = (fetchMock as unknown as { mock: { calls: [string, { method?: string; body?: string }][] } }).mock.calls
      .find(c => c[0] === '/api/organizations/o1/conformite/traitements/t3' && c[1]?.method === 'PATCH')
