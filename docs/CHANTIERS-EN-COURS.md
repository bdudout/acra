...
| Moyenne | IA gouvernée / MCP | **En cours — phase 1 (socle) livrée.** Interrupteurs d'instance **API v1 et MCP** (SUPER_ADMIN, **OFF par défaut**) + gate `isApiEnabled`/`isMcpEnabled` (fail-closed) + UI `/admin/instance`. **Cadrage validé** (`docs/mcp-cadrage.md`). **Phase 1 (§10.1)** : scope d'API **`mcp`** (moindre privilège, distinct de read/write ; case à cocher UI + i18n) ; **endpoint serveur MCP `/api/mcp`** (JSON-RPC 2.0, transport requête→réponse, sans dépendance ajoutée) gardé `mcpEnabled` + clé d'API + scope `mcp` + **org-scope strict** ; **rate limit** `mcp:<keyId>` ; **audit `MCP_TOOL_INVOKED`** (+ SIEM) par appel ; premier outil **`read_referentiels`** (lecture org-scopée). Cœur protocole **pur et testé** (`lib/mcp/protocol.ts`), handlers `.server.ts`. **Reste** (phases 2-5) : outils de contexte (`read_taxonomie`/`read_sector_examples`/`read_risk_posture`), **file de propositions** + validation UI, outils `propose_*`, tests IDOR MCP. |
