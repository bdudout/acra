...
| Moyenne | IA gouvernée / MCP | **En cours.** Interrupteurs d'instance **API v1 et MCP** (SUPER_ADMIN, **OFF par défaut**) : schéma + migration + gate `isApiEnabled`/`isMcpEnabled` (fail-closed) + UI `/admin/instance` + tests. **Cadrage MCP validé** (`docs/mcp-cadrage.md`) : ACRA = serveur MCP ; écritures = propositions validées en UI ; auth clé d'API + scope `mcp` ; usages intake/édition assistée/recommandations ; souveraineté (aucune donnée vers un LLM externe par défaut). **Reste** : scope `mcp`, endpoint `/api/mcp`, file de propositions, outils, journalisation MCP (phasage §10 du cadrage). |
